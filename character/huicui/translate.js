const translates = {
	...
	dcfeibai: "飞白",
	dcfeibai_info: "当你使用牌结算完毕后，你可以从随机两张字数为X的牌选择一张获得（X为此牌与你本回合使用的上一张牌的字数之和）。若牌堆中没有字数为X的牌，则你摸两张牌并标记为“弦”，然后〖飞白〗于本回合失效。",
	kuaiqi: "蒯祺",
	dcliangxiu: "良秀",
	...
	dcyizheng: "议政",
	dcyizheng_info: "你的回合开始和结束时，你可与任意名其他角色各展示一张手牌，若展示的牌类型均相同，你可将这些牌交给一名角色，否则，你弃置这些牌。",
	dcguilin: "归林",
	dcguilin_info: "限定技，出牌阶段或你进入濒死状态时，你可以回满体力并回复值张牌，然后失去〖议政〗并修改〖博玄〗。",
	dc_xiangchong: "新杀向宠",
	...
	dcgengdu: "耕读",
	dcgengdu_info: "出牌阶段开始时，你可以亮出牌堆顶四张牌，并选择一种颜色的所有牌获得。若你选择的颜色为：黑色，本阶段限X次，你使用黑色牌后摸两张牌，这些牌本回合无法使用或打出且不计入手牌上限；红色，本阶段限X次，你可以将一张红色牌当作本回合未使用过的普通锦囊牌使用。（X为本次你未获得的亮出牌数）",
	dcgumai: "孤脉",
	dcgumai_info: "每回合限一次，你造成或受到伤害时，若你有手牌，你可以展示所有手牌并令此伤害+1/-1。若展示的牌花色均相同，你可以弃置一张手牌令此技能视为未发动过。",
	jimiaojimu: "吉邈吉穆",
	dczouyi: "诹议",
	dczouyi_info: "出牌阶段限一次，你可执行至多两项：1.你摸2张牌并可弃置一名其他角色1张牌；2.你弃置1张牌并可令一名其他角色摸2张牌。结算后〖掩袭〗使用次数增加手牌数与你相同的角色数。",
	dcyanxi: "掩袭",
	dcyanxi_info: "每局游戏限零次，其他角色回合结束时，你可视为对其使用1张【杀】，结算后若此技能可使用次数不为0，再次发动直至此技能无使用次数或其阵亡。",
};

...
const dynamicTranslates = {
	...
	dcsbshimou(player) {
		...
		if (!player.storage.dcsbshimou) {
			str1 = `<span class=thundertext>${str1}</span>`;
		} else {
	...
	dcsbquanmou(player) {
		if (player.storage.dcsbquanmou) {
			return '转换技。①游戏开始时，你可以转换此技能状态；②出牌阶段每名角色限一次，你可以令一名攻击范围内的其他角色交给你一张牌。阳：当你于本阶段内下次对其造成伤害时，取消之；<span class="bluetext">阴：当你于本阶段内下次对其造成伤害后，你可以选择除其外的至多三名其他角色，对这些角色依次造成1点伤害。</span>';
		}

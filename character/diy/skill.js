...
const skills = {
	...
	diyjuxian: {
		audio: 2,
		trigger: {
			player: ["useCardToPlayered", "phaseDiscardBefore"],
		},
		forced: true,
		filter(event, player) {
			if (event.name != "phaseDiscard") {
				...
				return !player.hasHistory("sourceDamage", evt => evt.num) || !player.hasHistory("useCard", evt => get.tag(evt.card, "damage") > 0.5);
			}
		},
		getSix(all = false) {
			let cards = [];
			if (!all) {
				const card = get.cardPile2(card => get.number(card) == 6);
				if (card) {
					cards.push(card);
				}
			} else {
				const pile = ["cardPile", "discardPile"].map(pos => Array.from(ui[pos].childNodes)).flat();
				cards.addArray(pile.filter(card => get.number(card) == 6));
				game.players.forEach(target => cards.addArray(target.getCards("ej", card => get.number(card) == 6)));
			}
			return cards;
		},
		async content(event, trigger, player) {
			if (trigger.name != "phaseDiscard") {
				const cards = lib.skill[event.name].getSix();
				player.addTempSkill("diyjuxian_effect");
				if (cards.length > 0) {
					await player.gain(cards, "gain2").set("gaintag", ["diyjuxian"]);
				} else {
					await player.draw().set("gaintag", ["diyjuxian"]);
				}
			} else {
				trigger.cancel();
			}
		...
		subSkill: {
			effect: {
				onremove(player) {
					player.removeGaintag("diyjuxian");
				},
				charlotte: true,
				mod: {
					ignoredHandcard(card, player) {
						if (card.hasGaintag("diyjuxian")) {
							return true;
						}
					},
					cardDiscardable(card, player, name) {
						if (name == "phaseDiscard" && card.hasGaintag("diyjuxian")) {
							return false;
						}
					},
				},
	...
	diyshiji: {
		...
		mod: {
			cardUsable(card, player, num) {
				if (card.storage?.diyshiji) {
					return Infinity;
				}
				if (_status.event.name != "chooseToUse" || _status.event.skill) {
					return;
				}
				const storage = player.getStorage("diyshiji_effect"),
					hs = player.getCards("h");
				if (storage.length && card.cards.length == 1 && card.cards?.containsSome(...hs)) {
					return Infinity;
				}
			},
		...
		usable: 2,
		filter(event, player) {
			if (!player.countCards("hes", card => get.number(card, player) == 6)) {
				return false;
			}
			return (
				get.inpileVCardList(info => {
					return info[2] == "sha" && event.filterCard(get.autoViewAs({ name: info[2], nature: info[3], storage: { diyshiji: true } }, "unsure"), player, event);
				}).length > 0
			);
		},
		chooseButton: {
			dialog(event, player) {
				...
				return ui.create.dialog("识计", [list, "vcard"]);
			},
			check(button) {
				const player = get.player();
				return player.getUseValue(get.autoViewAs({ name: button.link[2], nature: button.link[3], storage: { diyshiji: true } }, "unsure"), true, false);
			},
			backup(links, player) {
				return {
					audio: "diyshiji",
			...
			prompt(links, player) {
				return `将一张点数为6的牌当做${get.translation(links[0][3]) || ""}${get.translation(links[0][2])}使用`;
			},
		},
		ai: {
			order(item, player) {
				const list = get
					.inpileVCardList(info => {
						...
						return info[2] == "sha" && player.hasValueTarget(card);
					})
					.map(info => get.autoViewAs({ name: info[2], nature: info[3] }, "unsure"));
				if (!list.length) {
					return 0;
				}
				return Math.max(...list.map(card => get.order(card) - 0.5));
			},
			result: {
				player: 1,
			},
		},
		group: ["diyshiji_viewAs"],
		subSkill: {
			viewAs: {
				audio: "diyshiji",
				trigger: { player: "useCardAfter" },
				filter(event, player) {
					return get.number(event.card) == 6 && get.type(event.card) != "equip";
				},
				prompt2(event, player) {
					return `摸一张牌，然后你下次使用的牌视为【${get.translation(event.card.name)}】（无距离次数限制）`;
				},
				async content(event, trigger, player) {
					player.addSkill("diyshiji_effect");
					player.markAuto("diyshiji_effect", trigger.card);
					await player.draw();
				},
			},
			effect: {
				...
				filter(event, player) {
					return player.getStorage("diyshiji_effect").length > 0;
				},
				async content(event, trigger, player) {
					const bool = trigger.modSkill.cardname == event.name || trigger.modSkill.cardnature == event.name; //evt.name == "chooseToUse" && !evt.skill && trigger.cards.length == 1
					if (trigger.addCount !== false && bool) {
	...
	diyzhanxian: {
		...
		async content(event, trigger, player) {
			...
			const cards = lib.skill.diyjuxian.getSix(true),
				discard = [],
				map = {};
			cards.forEach(card => {
				...
				if (get.owner(card)) {
					const id = get.owner(card).playerid;
					if (!map[id]) {
						map[id] = [];
					}
					map[id].push(card);
			...
			next.set("animate", event => {
				const player = event.player,
					...
					map = event.give_map,
					field = Object.values(map).flat();
				if (discard.length + field.length < cards.length) {
					var num = cards.length - discard.length - field.length;
					player.$draw(num);
					game.log(player, "从牌堆获得了", get.cnNumber(num), "张点数为6的牌");
				}
				if (discard.length > 0) {
					player.$gain2(discard, false);
					game.log(player, "从弃牌堆获得了", discard);
				}
				if (field.length > 0) {
					for (const id in map) {
						const source = (_status.connectMode ? lib.playerOL : game.playerMap)[id];
						source.$give(map[id], player);
					}
				}
				return 500;
			});
			await next;
			player.addTempSkill(event.name + "_damage", "phaseJieshuBegin");
		},
		subSkill: {
			damage: {
				audio: "diyzhanxian",
				...
				filter(event, player) {
					return get.number(event.card) == 6;
				},
				async cost(event, trigger, player) {
					event.result = await player
						.chooseTarget(`战贤：是否对一名角色造成一点伤害并摸一张牌`)

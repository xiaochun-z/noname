const translates = {
	...
	diy_wu_lvmeng: "武吕蒙",
	diy_wu_lvmeng_prefix: "武",
	diyjuxian: "据险",
	diyjuxian_info: "锁定技，当你使用牌指定唯一目标后，你从牌堆中获得一张点数为6的牌，若牌堆中没有点数为6的牌，你改为摸一张牌（你以此法获得的牌本回合不计入手牌上限）。若你于回合内未使用过伤害牌或未造成过伤害，你跳过本回合的弃牌阶段。",
	diyshiji: "识计",
	diyshiji_info: "出牌阶段限两次，你可将一张点数为6的牌当做任意【杀】使用（不计入次数）。你使用点数为6的非装备牌结算结束后，你可摸一张牌，然后你的手牌于下次需要使用牌时均视为上一张使用过的牌（无距离次数限制）。",
	diyzhanxian: "战贤",
	diyzhanxian_info: "限定技，出牌阶段，你可从场上、牌堆或弃牌堆中获得所有点数为6的牌。直到本回合结束阶段，你每使用一张点数为6的牌后，你可对一名角色造成一点伤害并摸一张牌。",
};
